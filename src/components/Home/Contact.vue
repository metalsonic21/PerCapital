<template>
<div id="contacto" class="contact-mine">
    <div class="block-section">
        <div class="container">

            <div class="navy-line" style="opacity:0"></div>
            <h1 align="center" class="title-mine section-title-per">{{$t('contact')}}</h1>

            <br>
            <div class="row">
                <div class="col-xs-12 col-md-4 col-sx-4">

                    <div class="contact-box">
                        <font-awesome-icon icon="map-marker" size="3x" style="color:grey"></font-awesome-icon>
                        <br><br>
                        <h4 class="title-mine contact-title">{{$t('location')}}</h4>
                        <p>Av. Circunvalación del Sol, Torre Santa Paula Plaza II, Ofic. 104<br>Caracas, Venezuela</p>
                    </div>
                </div>
                <div class="col-xs-12 col-md-4 col-sx-4">
                    <div class="contact-box">
                        <font-awesome-icon icon="phone" size="3x" style="color:grey;-webkit-transform: scaleX(-1);transform: scaleX(-1);"></font-awesome-icon>
                        <br><br>
                        <h4 class="contact-title">{{$t('phone')}}</h4>
                        <p></p>
                        <br><br>
                    </div>
                </div>
                <div class="col-xs-12 col-md-4 col-sx-4">
                    <div class="contact-box">
                        <font-awesome-icon icon="envelope" size="3x" style="color:grey"></font-awesome-icon>
                        <br><br>
                        <h4 class="contact-title">{{$t('email')}}</h4>
                        <p>info@percapital.com<br><br></p>
                        <br>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>

</template>

<style>

</style>
